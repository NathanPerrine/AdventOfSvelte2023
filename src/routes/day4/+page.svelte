<script lang="ts">
  ...
  $: if (heartRates){
    ...
    averageHeartRate = heartRates.map(heartRate => heartRate.heartRate)
    ?.slice(Math.max(heartRates.length + timeMax, 0)).reduce((acc, val) => acc + val, 0) / (timeFilter === 'all' ? heartRates.length : -timeMax)
  }

  $: lineLabels = heartRates.map(heartRate => heartRate.time).slice(Math.max(heartRates.length + timeMax, 0))
  $: lineData = heartRates.map(heartRate => heartRate.heartRate).slice(Math.max(heartRates.length + timeMax, 0))
...
<div class="mt-8 mx-auto w-11/12">
	<div class="w-full flex justify-around">
		<div class="card w-48 h-64 bg-base-200 shadow-xl border border-primary">
			...
			<div class="card-body items-center text-center">
				<h2 class="card-title">Current Heart Rate</h2>
				{#if currentHeartRate}
		...
		<div class="card w-48 h-64 bg-base-200 shadow-xl border border-primary">
			...
			<div class="card-body py-4 items-center text-center">
				<h2 class="card-title">Average Heart Rate:</h2>
				{#if averageHeartRate}
          {#if timeFilter === 'all'}
            <p>All time</p>
  ...
  <div class="flex justify-center md:justify-around flex-wrap">
    <button on:click={allTime}      class="btn btn-secondary btn-sm md:btn-md w-24 m-2" class:!btn-primary={timeFilter == 'all'}>All Time</button>
    <button on:click={tenSeconds}   class="btn btn-secondary btn-sm md:btn-md w-24 m-2" class:!btn-primary={timeFilter == -10}>Last 10 seconds</button>
    <button on:click={sixtySeconds} class="btn btn-secondary btn-sm md:btn-md w-24 m-2" class:!btn-primary={timeFilter == -60}>Last 60 seconds</button>
  </div>
</div>
